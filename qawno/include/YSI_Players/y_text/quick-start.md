...
new
    gpDataEngkq[MAX_PLAYERS][E_PLAYER_DATA];
...
stock Func()
{
    Text_Send(@ gpDataEngkq<E_PLAYER_DATA_ADMIN>, $SOME_MESSAGE);
}
```
This specifies that "gpDataEngkq" is a 2d enum array and only the "E_PLAYER_DATA_ADMIN" slot should be used. It should be noted that this line MAY give a warning about index tag mismatches, I don't know how to avoid this so just ignore it.
